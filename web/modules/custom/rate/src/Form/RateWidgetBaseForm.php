...
class RateWidgetBaseForm extends ContentEntityForm {
  ...
  private function getRateOptions($options, $value_type, $template) {
    $rate_options = [];
  ...
  private function getVotesSumm($form_state) {
    $entity = $this->getEntity();
  ...
  private function prepareValueFivestar($template, &$form, FormStateInterface $form_state) {
  ...
  private function prepareValueNumberupdown($template, &$form, FormStateInterface $form_state) {
    $this->prepareValue($template, $form, $form_state);
    $options = $form_state->get('options');
    $all_votes = $this->getVotesSumm($form_state);
    foreach (array_slice($options, 1) as $key => $option) {
      if (!isset($results['result_type']) || $results['result_type'] === 'vote_hidden') {
        continue;
      }
  ...
  private function prepareValue($template, &$form, FormStateInterface $form_state) {
    $entity = $this->getEntity();
    $options = $form_state->get('options');
    $value_type = $entity->get('value_type')->value;
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $rate_options = $this->getRateOptions($options, $value_type, $template);
    ...
    switch ($template) {
      case 'numberupdown':
        $this->prepareValueNumberupdown($template, $form, $form_state);
        break;
      case 'fivestar':
        $this->prepareValueFivestar($template, $form, $form_state);
        break;
      default:
        $this->prepareValue($template, $form, $form_state);
    }
    ...
    $form['value']['#deadline_disabled'] = $deadline_disabled;
    ...
    if (isset($results['result_position']) && $results['result_position'] !== 'hidden') {
      ...
      $form['result'] = [
        ...
        '#children' => [
          'result' => $plugin->getVoteSummary($entity) + [
              '#disabled' => $form['value']['#disabled'],
              '#deadline_disabled' => $deadline_disabled,
            ],
        ],
        '#weight' => 100,
      ];
  ...
  private function ajaxSubmitNumberUpDown(&$form, FormStateInterface $form_state) {
    $user_id = $this->account->id();
    $plugin = $form_state->get('plugin');
    $entity = $this->getEntity();
    $value_type = $entity->get('value_type')->value;
    ...
    if (isset($results['result_type'])) {
      ...
      $bundle = $form_state->get('entity_bundle');
      $entity_id = $form_state->get('entity_id');


      switch ($results['result_type']) {
        case 'user_vote_empty':
          $votes = $plugin->getVotes($entity_type, $bundle, $entity_id, $vote_type, $value_type, $rate_widget, $user_id);
          break;
        case 'user_vote_average':
          $votes = $plugin->getVotes($entity_type, $bundle, $entity_id, $vote_type, $value_type, $rate_widget, $user_id);
          if (count($votes) === 0) {
            $votes = $plugin->getVotes($entity_type, $bundle, $entity_id, $vote_type, $value_type, $rate_widget);
          }
          break;
        default:
          $votes = $plugin->getVotes($entity_type, $bundle, $entity_id, $vote_type, $value_type, $rate_widget);
  ...
  private function ajaxSubmitFivestar(&$form, FormStateInterface $form_state) {
    $settings = $form_state->get('settings');
    $entity = $this->getEntity();
    $results = $settings->get('results');
    $entity_value = $entity->getValue();
  ...
  public function ajaxSubmit(array $form, FormStateInterface $form_state) {
    ...
    if (isset($template)) {
      if (isset($results['result_type']) && $results['result_type'] != 'vote_hidden') {
        ...
        switch ($template) {
          case 'fivestar':
            $this->ajaxSubmitFivestar($form, $form_state);
            break;
          case 'numberupdown':
            $this->ajaxSubmitNumberUpDown($form, $form_state);
            break;
          default:
            foreach ($options as $key => $option) {
              if ($value_type == 'option') {
